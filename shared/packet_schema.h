...
// ESP-NOW command bytes (sent from base node to sensor nodes)
#define CMD_SET_RADIUS    0x01
#define CMD_SET_CONE      0x02
#define CMD_CONE_LOCK     0x03
// CMD_SYNC_CHANNEL: payload is [0x04, channel_number (1-13)]
// Locks all nodes to the same channel for multi-node RSSI correlation.
// Send with only the command byte (len=1) to resume autonomous hopping.
#define CMD_SYNC_CHANNEL  0x04